...
namespace DigitalPal.DataAccess.Util
{
    public sealed class DynamicQuery
    {
        ...
        public static QueryResult GetDynamicPageQuery<T>(string tableName, int pageNumber, int pageSize,
            string sortProperty, bool sortDescending, Expression<Func<T, bool>> expression)
        {
        ...
        private static void WalkTree(System.Linq.Expressions.Expression body, ExpressionType linkingType,
        ...
        private static void WalkContainsMethod(MethodCallExpression body, ExpressionType linkingType,
            ref List<QueryParameter> queryProperties)
        {
            // There are two ways Contains can be used
            // 1. x => x.FirstName.Contains(searchText)
            // 2. x => ids.Contains(x.Id)
            //

            if (body.Arguments.Count == 1) // This handles first use case, we need some robust decinding logic here
            {
